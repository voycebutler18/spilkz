...
const Favorites = () => {
  ...
  const fetchFavorites = async () => {
    try {
      ...
      const userIds = [...new Set((spliksData || []).map((s: any) => s.user_id))];
      const { data: profilesData } = await supabase
        .from("profiles")
        .select("id, username, display_name, avatar_url")
        .in("id", userIds);

      const transformed = favoritesList
        .map((fav) => {
          const splik: any = spliksData?.find((s: any) => s.id === fav.splik_id);
          if (!splik) return null;
          const profile = profilesData?.find((p: any) => p.id === splik.user_id);
          return {
            id: fav.id,
            splik_id: fav.splik_id,
            created_at: fav.created_at,
            splik: { ...splik, profile: profile || undefined },
          } as Favorite;
        })
  ...
  /** NEW: go straight to the creator page pointing at this video */
  const openOnCreator = (fav: Favorite) => {
    const slug = fav.splik.profile?.username || fav.splik.user_id; // fallback if no username
                ...
                <div className="relative aspect-[9/16] bg-black">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-3 text-white">
                    ...
                    <p className="text-xs opacity-80">
                      {fav.splik.views?.toLocaleString() || 0} views
                    </p>
                  </div>
                </div>
                    ...
                    <div className="flex items-center gap-2">
                      {fav.splik.profile?.avatar_url && (
                        <img
                          ...
                          alt={fav.splik.profile.display_name}
                          className="w-6 h-6 rounded-full"
                        />
                      )}
                      <span className="text-sm text-muted-foreground truncate">
                        {fav.splik.profile?.display_name || "Unknown"}
                      </span>
                    </div>
                    <Button
                      variant="ghost"
                  ...
                  <div className="flex gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-4 text-sm text-muted-foreground">
                        <span>{fav.splik.views?.toLocaleString() || 0} views</span>
                        <span>{fav.splik.likes_count?.toLocaleString() || 0} likes</span>
                      ...
                      <div className="flex items-center justify-between mt-3">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm">
                            {fav.splik.profile?.display_name || "Unknown Creator"}
                          </span>
                        </div>
                        <Button
                          variant="ghost"
