...
export default function ProfilePage() {
  ...
  // load stats (public: visible to anyone)
  useEffect(() => {
    let cancelled = false;
    ...
    const loadStats = async () => {
      ...
      try {
        ...
        // Total boosts received across their spliks
        let boostsCount = 0;
        const { data: spliks, error: spliksErr } = await supabase
          .from("spliks")
          .select("id")
          .eq("user_id", profile.id)
          .limit(1000); // adjust if you expect >1000 posts
        ...
        console.warn("Stats load failed (RLS/policies might block public reads):", e);
        if (!cancelled) {
          setFollowers(0);
          setFollowing(0);
          setTotalBoosts(0);
    ...
    // Live updates for followers (boosts are left as snapshot like the creator page)
    const ch = profile?.id
      ? supabase
