...
function SimpleUploader() {
  ...
  const uploadMedia = async (file: File): Promise<{ url: string; thumbnailUrl?: string }> => {
    ...
    const filePath = `spliks/${fileName}`;
    ...
    if (file.type.startsWith('video/')) {
      try {
        ...
        const thumbFileName = `spliks/thumb_${fileName.replace(/\.[^/.]+$/, '.jpg')}`;
    ...
    try {
      ...
      if (mediaFile) {
        ...
        if (mediaType === 'video') {
          ...
          trimStart = 0;
          trimEnd = 3; // 3-second limit
        } else {
          // For photos, store in thumbnail_url
  ...
  return (
    <div className="rounded-lg border bg-card p-4 space-y-4">
      ...
      <div>
        <Textarea
          placeholder="Add a description or prayer..."
          value={description}
      ...
      {mediaPreview && (
        <div className="relative rounded-lg overflow-hidden bg-gray-100">
          ...
          {/* 3-second limit indicator for videos */}
          {mediaType === 'video' && (
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <Button
          ...
          disabled={uploading || (!mediaFile && !title.trim() && !description.trim())}
      ...
      <div className="text-xs text-muted-foreground">
        Share photos, 3-second videos, prayers, testimonies, or inspirational content. This will appear in the main feed.
      </div>
    </div>
  );
}
