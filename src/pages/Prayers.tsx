...
export default function PrayerMediaUpload({ open, onOpenChange, onUploaded }: PrayerMediaUploadProps) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // Debug: Test bucket access first
      console.log("Testing bucket access...");
      const { data: buckets, error: bucketsError } = await supabase.storage.listBuckets();
      console.log("Available buckets:", buckets);
      if (bucketsError) {
        console.error("Bucket list error:", bucketsError);
        toast.error(`Bucket access error: ${bucketsError.message}`);
        return;
      }

      const bucketExists = buckets?.find(b => b.name === PRAYER_BUCKET);
      if (!bucketExists) {
        toast.error(`Bucket '${PRAYER_BUCKET}' not found in available buckets: ${buckets?.map(b => b.name).join(', ')}`);
        return;
      }
      ...
      console.log("Uploading to bucket:", PRAYER_BUCKET, "filename:", fileName);
