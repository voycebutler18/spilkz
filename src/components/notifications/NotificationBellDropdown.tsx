...
export default function NotificationBellDropdown({ user }: Props) {
  ...
  const [cursor, setCursor] = useState<string | null>(null); // created_at of last loaded row
  ...
  const fetchPage = async (uid: string, olderThan?: string | null) => {
    const q = supabase
      .from("notifications")
      .select("*")
      .eq("recipient_id", uid)
    ...
    // fetch any missing actor profiles (best-effort)
    const missingActorIds = Array.from(
      new Set(rows.map(r => r.actor_id).filter((id): id is string => !!id && !actors[id]))
    );
    if (missingActorIds.length) {
      ...
      if (Object.keys(map).length) setActors(m => ({ ...m, ...map }));
    }
    return { rows };
  };
  ...
  if (!user?.id) {
    return (
      <Button variant="ghost" size="icon" aria-label="Notifications" onClick={() => navigate("/login")} title="Notifications">
  ...
  return (
    <DropdownMenu open={open} onOpenChange={setOpen}>
      ...
      <DropdownMenuContent align="end" className="w-80 p-0">
          ...
          <ul className="px-1 py-1">
            {loading ? (
              ...
              items.map(row => {
                ...
                const initial = (actor?.display_name?.[0] || actor?.username?.[0] || "U").toUpperCase();
                return (
                  <li
                    key={row.id}
                  ...
                  >
                    <Avatar className="h-8 w-8 shrink-0">
                      {actor?.avatar_url ? <AvatarImage src={actor.avatar_url} alt="" /> : <AvatarFallback>{initial}</AvatarFallback>}
                    </Avatar>
        ...
        <div className="border-t px-3 py-2 text-xs text-muted-foreground">New items appear here instantly.</div>
      </DropdownMenuContent>
    </DropdownMenu>
  );
}
