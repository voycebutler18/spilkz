...
const BUCKET_FOR_KIND: Record<string, string> = {
  ...
  food_image: "food-images", // add this bucket later when wiring Food
};
...
export default function RightSiteHighlights({
  ...
  const prev = () =>
    setLightIdx(i => (i === null ? i : (i + imageItems.length - 1) % imageItems.length));
  const next = () =>
    setLightIdx(i => (i === null ? i : (i + 1) % imageItems.length));
    ...
    <aside className="hidden lg:block sticky top-4 h-[calc(100vh-2rem)] overflow-auto">
      ...
      {lightOpen && (
        <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-3 sm:p-6">
          ...
          <img
            src={getImageUrl(
              imageItems[lightIdx!].kind,
              imageItems[lightIdx!].image_path!
            )}
            alt=""
