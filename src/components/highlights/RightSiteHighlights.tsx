...
type Highlight = {
  ...
  route: string; ref_table: string; ref_id: string;
  image_path: string | null; text_preview: string | null; mood: string | null;
  ...
  React.useEffect(() => {
    ...
    const loadFromHighlights = async () => {
      const { data, error } = await supabase
        .from("site_highlights_active").select("*")
        .in("kind", includeKinds as any)
        .order("created_at", { ascending: false })
        .limit(limit);
      if (error) throw error;
      if (alive) setItems((data as Highlight[]) ?? []);
    };
    ...
    const fallbackFromThoughts = async () => {
      ...
      const { data: posts } = await supabase
        .from("thoughts_posts").select("id, text_content, mood, created_at")
        .gt("created_at", sinceISO).order("created_at", { ascending: false }).limit(limit);

      const photoItems: Highlight[] = (photos ?? []).map((p: any) => ({
        id: p.id, user_id: null, kind: "thought_image", route: `/thoughts/photos/${p.id}`,
        ref_table: "thoughts_images", ref_id: p.id, image_path: p.path, text_preview: null, mood: null,
        created_at: p.created_at, expires_at: new Date(Date.parse(p.created_at) + 86400000).toISOString(),
      }));

      const postItems: Highlight[] = (posts ?? [])
        .filter((t: any) => (t.text_content ?? "").trim().length > 0)
        .map((t: any) => ({
          id: t.id, user_id: null, kind: "thought_post", route: `/thoughts`,
          ref_table: "thoughts_posts", ref_id: t.id, image_path: null,
          text_preview: (t.text_content ?? "").slice(0, 160), mood: t.mood ?? null,
          created_at: t.created_at, expires_at: new Date(Date.parse(t.created_at) + 86400000).toISOString(),
        }));

      if (alive) setItems([...photoItems, ...postItems].sort((a, b) => b.created_at.localeCompare(a.created_at)).slice(0, limit));
    };
    ...
    <>
      <div className="rounded-2xl border border-neutral-800 bg-neutral-900 p-4">
        ...
        <div className="mt-4 flex flex-col gap-3">
          {items.length === 0 && <p className="text-sm text-neutral-400">New photos & status posts will appear here for 24 hours.</p>}
          ...
          {items.map(h => {
            ...
            if (isImage) {
              ...
              return (
                <button key={h.id} onClick={() => openAtId(h.id)} className="flex items-center gap-3 text-left group" aria-label="Open photo">
            ...
            return (
              <button key={h.id} onClick={() => navigate(h.route)} className="flex items-center gap-3 text-left rounded-xl px-2 py-2 hover:bg-white/5 transition" aria-label="Open post">
