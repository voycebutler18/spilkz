...
export default function SplikCard(props: SplikCardProps) {
  ...
  /* ---- keep counts in sync if parent provides new row ---- */
  useEffect(() => {
    setLikesCount(splik.likes_count ?? 0);
    setCommentsCount(splik.comments_count ?? 0);
  }, [splik.id, splik.likes_count, splik.comments_count]);
    ...
    const io = new IntersectionObserver(
      async (entries) => {
        entries.forEach(async (entry) => {
          ...
          if (mostlyVisible && props.onPrimaryVisible)
  ...
  /* ---- state: user flags + realtime on spliks counters ---- */
  useEffect(() => {
    let mounted = true;
    ...
    (async () => {
      ...
      // realtime: listen to counter updates on spliks row
      const countersChannel = supabase
        .channel(`spliks-counters-${splik.id}`)
        .on(
          "postgres_changes",
          {
            event: "UPDATE",
          ...
          (payload) => {
            const next = payload.new as Partial<Splik>;
            if (typeof next.likes_count === "number")
              setLikesCount(next.likes_count);
            if (typeof next.comments_count === "number")
              setCommentsCount(next.comments_count);
          }
        )
        .subscribe();
  ...
  const handleSplik = async (e?: React.MouseEvent) => {
    ...
    try {
      ...
      onSplik?.();
      // final number will arrive via realtime update on spliks.likes_count
    } catch {
