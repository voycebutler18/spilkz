...
export function VideoGrid({
  ...
  // ✅ only track likes now (no views)
  const [videoStats, setVideoStats] = useState<Record<string, { likes: number }>>({});
  ...
  useEffect(() => {
    // seed likes for each card
    const stats: Record<string, { likes: number }> = {};
    spliks.forEach((s) => {
      stats[s.id] = { likes: s.likes_count || 0 };
    });
    setVideoStats(stats);

    // live updates for likes only
  ...
  const handleLike = async (splikId: string) => {
    ...
    setVideoStats((prev) => ({
      ...prev,
      [splikId]: { likes: Math.max(0, (prev[splikId]?.likes || 0) + (liked ? -1 : 1)) },
    }));
