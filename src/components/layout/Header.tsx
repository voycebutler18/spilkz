...
import {
  DropdownMenu,// src/components/layout/Header.tsx
import * as React from "react";
import { Link, useLocation, useNavigate } from "react-router-dom";
import { Sparkles, LogOut, Menu, Home, MessageSquare, Upload } from "lucide-react";
import { Button } from "@/components/ui/button";
    ...
    <header className="sticky top-0 z-50 w-full border-b bg-background/90 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="mx-auto flex h-14 max-w-7xl items-center gap-3 px-3 sm:px-4">
        ...
        <nav className="ml-auto hidden items-center gap-2 md:flex">
          ...
          {/* Upload -> open modal via context (no /upload route) */}
          <Button
            size="sm"
            className="gap-2"
            onClick={() => openUpload({ onCompleteNavigateTo: "/dashboard" })}
            aria-label="Upload"
            title="Upload a 3-second Splik"
            type="button"
          >
            ...
            >
              ...
              <nav className="flex flex-col gap-3 p-4">
                ...
                <Button
                  ...
                  onClick={() => {
                    setOpen(false);
                    // Open modal via context (auth gating handled inside)
